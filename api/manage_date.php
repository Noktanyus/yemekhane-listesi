...
try {
    ...
    if ($is_special_day) {
        ...
        foreach ($meal_names as $meal_name) {
            if (empty(trim($meal_name))) continue;
            
            $stmt_get_meal->execute([$meal_name]);
            $meal_id = $stmt_get_meal->fetchColumn();
            
            if (!$meal_id) {
                // Güvenlik: İsteğe bağlı olarak, bulunamayan yemekler için hata verebilir veya yoksayabilirsiniz.
                // Şimdilik yoksayıyoruz.
                continue;
            }
    ...
    // DÜZELTME: Hatalı create_log() fonksiyonu yerine log_action() kullanıldı.
