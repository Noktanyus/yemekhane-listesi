...
try {
    ...
    if ($method === 'GET') {
        switch ($action) {
            case 'get_single':
                $id = $_GET['id'] ?? null;
                if (!$id) throw new Exception('Yemek ID\'si bulunamadı.');
    ...
    elseif ($method === 'POST') {
        ...
        switch ($action) {
            ...
            case 'update':
                ...
                if (empty($name)) throw new Exception('Yemek adı boş olamaz.');
                ...
                if ($action === 'update') {
                    ...
                    if (!$id) throw new Exception('Güncellenecek yemek ID\'si bulunamadı.');
            ...
            case 'delete':
                $id = $_POST['id'] ?? null;
                if (!$id) throw new Exception('Silinecek yemek ID\'si bulunamadı.');
